...
# Генерация и отображение строки дней рождения через запятую:
# print(f'Здесь  {numBDays} дня рождения:')
print('Здесь',  numBDays, 'дня рождения:')
birthdays = getBirthdays(numBDays)
for i, birthday in enumerate(birthdays):
    if i != 0:
        # вывести запятую для каждого ДР после первого
        print(', ', end='')
    monthName = MONTHS[birthday.month - 1]  # индекс начинается с 0 ?
    dateText = f'{monthName} {birthday.day}'
    print(dateText, end='')     # добавить в строку новую дату
print('\n' * 2)

# Есть ли совпадающие ДР
match = getMatch(birthdays)     # возвращает повторяющуюсю дату если есть (иначе None)

# вывод результатов
print('В этой симуляции, ', end='')
if match != None:  # то есть имеется совпадение
...
for i in range(100_000):
    # отображаем сообщение о ходе выполнения каждые 10_000 операций
    if i % 10_000 == 0:
        print(i, 'симуляций запущено...')
    birthdays = getBirthdays(numBDays)  # каждый проход генерируем новый список случайных ДР
    if getMatch(birthdays) != None:     # если совпадение найдено
        simMatch = simMatch + 1         # увеличим счетчик (подсчет совпадений)
print('100.000 симуляций запущено.')

# вывод результатов имитационного моделирования
probability = round(simMatch / 100_000 * 100, 2)
print('За 100.000 симуляций, для', numBDays, 'людей, имеются совпадающие дни рождения в этой группе', simMatch, 'раз.')
